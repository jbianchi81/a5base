...
import { Interval, interval2string, IntervalDict } from './timeSteps'
...
export default class Variable extends baseModel  {
    ...
    static upsertVarQuery(variable : Variable) {
		var query = "\
	...
	async find() : Promise<Variable|undefined> {
		if(this.id) {
	...
	static async delete(filter={},options={}) : Promise<Variable[]> {
		var matches = await this.read(filter,{})
		if(!matches.length) {
			console.log("Nothing to delete")
			return []
		}
		const deleted : Variable[] = []
