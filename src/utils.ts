...
export async function delay(t : number, val? : unknown) {
...
function assertValidDateTruncField(field : string) {
	if ([
		"microseconds",
		"milliseconds",
		"second",
...
export function control_filter2(
	...
	for(const key in valid_filters) {
        if (Object.prototype.hasOwnProperty.call(valid_filters, key)) {
            ...
            if(filter[key] as any instanceof not_null) {
                ...
                if(valid_filter!.type == "regex_string") {
                    ...
                    if(filter[key] as any instanceof Geometry) {
...
export function pasteIntoSQLQuery(query : string, params : any[]) {
