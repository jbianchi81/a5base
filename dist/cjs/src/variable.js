...
class Variable extends baseModel_1.baseModel {
    constructor(args) {
        super(args);
    }
    async getId() {
    ...
    static upsertVarQuery(variable) {
        ...
        var params = [variable.id, variable["var"], variable.nombre, variable.abrev, variable.type, variable.datatype, variable.valuetype, variable.GeneralCategory, variable.VariableName, variable.SampleMedium, variable.def_unit_id, (0, timeSteps_1.interval2string)(variable.timeSupport)];
        return (0, utils_1.pasteIntoSQLQuery)(query, params);
    }
    async create() {
        await this.getId();
    ...
    static async read(filter = {}, options) {
        if (filter.id && !Array.isArray(filter.id)) {
            ...
            var filter_string = (0, utils_1.control_filter2)(valid_filters, filter, undefined, true);
